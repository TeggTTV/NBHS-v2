"use strict";
class NodeElement {
    constructor(parent, name, x, y, w, h, truthTable) {
        this.focused = false;
        this.dragging = false;
        this.setRel = false;
        this.relativeX = 0;
        ...
        if (this.truthTable) {
            ...
            console.log(`Truth table inputs: ${numInputs}, outputs: ${numOutputs}`);
            // Create input nodes
            let inputs = [];
    ...
    static loadFromLocalStorage(parent) {
        const customElements = JSON.parse(localStorage.getItem("savedTruthTables") || "[]");
        return customElements.map((data) => {
            const nodeElement = new NodeElement(parent, data.name, 0, 0, 100, 100, data.truthTable);
